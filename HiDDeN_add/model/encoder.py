...
class Encoder(nn.Module):
    ...
    def __init__(self, config: HiDDenConfiguration):
        ...
        add_layer= []
        for _ in range(config.encoder_blocks_add):
            layer = ConvBNRelu(self.conv_channels, self.conv_channels)
            add_layer.append(layer)

        add_layer.append(nn.Conv2d(self.conv_channels, 3, kernel_size=1))
        
        self.final_layer= nn.Sequential(*add_layer)
