...
class Hidden:
    ...
    def train_on_batch(self, batch: list):
        ...
        self.encoder_decoder.set_val_false()
        ...
        with torch.enable_grad():
            ...
            # train on fake
            encoded_images, noised_images, decoded_messages,mu,log_var = self.encoder_decoder(images, messages)
            ...
            loss_vib=torch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)
            g_loss = self.config.adversarial_loss * g_loss_adv + self.config.encoder_loss * g_loss_enc \
                     + self.config.decoder_loss * g_loss_dec+self.config.vib_loss_beta*loss_vib
    ...
    def validate_on_batch(self, batch: list):
        ...
        batch_size = images.shape[0]
        self.encoder_decoder.set_val_true()
